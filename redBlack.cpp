...
Nodo *insertarAux(Nodo *&T, int n, Color c)
{
    Nodo *recent = NULL;
    ...
    if (n <= T->dato)
    {
        if (T->izq == NULL)
        {
            recent = crearNodo(n, c);
            recent->parent = T;
            T->izq = recent;
        }
        else
            recent = insertarAux(T->izq, n, c);
    }
    else
    {
        if (T->der == NULL)
        {
            ...
            recent = insertarAux(T->der, n, c);
    }

    return recent;
}

void insertar(Nodo *&T, int n, Color c)
{
    if (estaVacio(T))
    {
        T = crearNodo(n, c);
    ...
    {
        Nodo *nuevo = insertarAux(T, n, c);
        if (nuevo->parent->col != BLACK)
            fixDoubleRed(T, nuevo);
    }
}
...
Color borrarAux(Nodo *&T, int n, Nodo *&recent, Nodo *&daddy)
{
    ...
    if (n < T->dato)
        return borrarAux(T->izq, n, recent, daddy);
    else
        return borrarAux(T->der, n, recent, daddy);
}
...
int main()
{
    ...
    while (n--)
    {
        ...
        insertar(T, numer, colorcito);
    }
    //si se quiere probar con el caso de prueba
    /*
    insertar(T, 50, BLACK);
    insertar(T, 40, RED);
    insertar(T, 30, BLACK);
